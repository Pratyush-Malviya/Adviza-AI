...
export default async function ActionsPage() {
  ...
  const renderActionItem = (item: ActionItemRow) => {
    ...
    return (
      <div key={item.id} className="bg-white rounded-3xl p-5 border border-[#EADBCE] shadow-sm flex items-start gap-4 hover:shadow-md transition-all">
        <div className={cn("w-2.5 h-2.5 rounded-full flex-shrink-0 mt-2",
          item.status === "completed" ? "bg-emerald-500" : item.priority === "high" ? "bg-rose-500" : item.priority === "medium" ? "bg-amber-500" : "bg-zinc-400"
        )} />
        <div className="flex-1 min-w-0">
          <p className={cn("text-sm font-bold", item.status === "completed" ? "line-through text-[#8E847C]" : "text-[#121217]")}>
            {item.description}
          </p>
          <div className="flex items-center flex-wrap gap-3 mt-2">
            <span className={cn("text-xs font-bold px-2.5 py-0.5 rounded-full border uppercase", config.color)}>{item.priority}</span>
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto">
      <div>
        ...
        <p className="text-sm text-[#7A726A] mt-1">{open.length} open · {inProgress.length} in progress · {completed.length} completed</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-5">
        {[
          ...
          const Icon = s.icon;
          return (
            <div key={s.label} className="bg-white rounded-3xl p-6 border border-[#EADBCE] shadow-sm flex items-center gap-4">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border ${s.bg} ${s.color} flex-shrink-0`}>
                <Icon className="w-6 h-6" />
              </div>
              <div>
                <div className="text-3xl font-heading font-extrabold text-[#121217]">{s.count}</div>
      ...
      {open.length > 0 && (
        <div>
          <h2 className="text-xs font-heading font-bold text-rose-700 uppercase tracking-wider mb-4 flex items-center gap-2">
            <AlertCircle className="w-4 h-4" />Open Actions ({open.length})
          </h2>
          <div className="space-y-3">{open.map(renderActionItem)}</div>
        </div>
      )}
      ...
      {inProgress.length > 0 && (
        <div>
          ...
          <div className="space-y-3">{inProgress.map(renderActionItem)}</div>
        </div>
      )}
      ...
      {completed.length > 0 && (
        <div>
          <h2 className="text-xs font-heading font-bold text-[#8E847C] uppercase tracking-wider mb-4 flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4" />Completed ({completed.length})
          </h2>
          <div className="space-y-3 opacity-75">{completed.map(renderActionItem)}</div>
        </div>
      )}
