...
export function WorkflowCanvas({
  ...
  // Auto-Layout Nodes using topological layering
  ...
  // Sequential Live Execution Simulation
  const handleSimulateLiveExecution = async () => {
    if (isSimulating || nodes.length === 0) return;
    setIsSimulating(true);

    // Reset status to idle
    let currentNodes = nodes.map((n) => ({ ...n, executionStatus: "idle" as ExecutionStatus }));
    onUpdateNodes(currentNodes);

    for (let i = 0; i < currentNodes.length; i++) {
      const targetId = currentNodes[i].id;
      currentNodes = currentNodes.map((n) =>
        n.id === targetId ? { ...n, executionStatus: "running" as ExecutionStatus } : n
      );
      onUpdateNodes(currentNodes);
      ...
      currentNodes = currentNodes.map((n) =>
        n.id === targetId ? { ...n, executionStatus: "success" as ExecutionStatus } : n
      );
      onUpdateNodes(currentNodes);
    }
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 right-4 z-20 flex items-center gap-1.5 bg-white/95 backdrop-blur-md p-1.5 rounded-2xl border border-[#EADBCE] shadow-md pointer-events-auto">
        {/* Simulation & Auto-Layout actions */}
        <button
          onClick={handleSimulateLiveExecution}
          disabled={isSimulating || nodes.length === 0}
          title="Simulate Step-by-Step Live Execution"
          className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-600 hover:bg-rose-500 text-white rounded-xl text-xs font-semibold transition disabled:opacity-50 cursor-pointer shadow-xs mr-1"
        >
        ...
        <button
          ...
          title="Auto-Layout Nodes (Topological Alignment)"
          className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-semibold text-[#645F5A] hover:text-[#121217] hover:bg-[#FAF5F0] rounded-xl transition cursor-pointer border border-[#EADBCE] mr-1"
        >
